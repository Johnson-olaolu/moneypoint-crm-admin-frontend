...
const Login = () => {
  ...
  const loginMutation = useMutation(async (loginData: { email: string; password: string }) => {
    const response: { user: IUser; customerSupport?: ICustomerSupport } = await authService.login(loginData);
    console.log(response);
    dispatch(userLogin(response));
    switch (response.user.role.name) {
      case userRoles.CUSTOMER_SERVICE:
        navigate("/ticket");
        break;
      default:
        break;
